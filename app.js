...
function initIcons() {
    if (typeof lucide !== 'undefined' && lucide.createIcons) {
        // Удаляем все ранее созданные SVG-иконки (Lucide добавляет атрибут data-lucide)
...
const apply=(b,ex)=>{ b.innerHTML=ex?'<i data-lucide="check"></i>':'<i data-lucide="calendar"></i>'; b.classList[ex?'add':'remove']('calendar-btn-added'); b.title=ex?'Уже в календаре':'Добавить в календарь'; };
...
async function addToCalendar(opName, planDate, planMileage) {
 ...
 const b=document.querySelector(`.calendar-btn[data-op-name="${opName}"][data-plan-date="${planDate}"]`); if(b){ b.innerHTML='<i data-lucide="check"></i>'; b.classList.add('calendar-btn-added'); b.title='Уже в календаре'; }
...
// === Экспорт в Excel (полная резервная копия, 7 листов) ===
...
// === Универсальный обработчик (выбор формата) ===
