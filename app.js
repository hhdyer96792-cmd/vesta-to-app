...
let tireLog = [];
let workCosts = [];
let serviceRecords = []; // для проверки дублирования
...
async function loadHistory() {
    ...
    try {
        ...
        serviceRecords = historyData.map(row => ({
            operation_id: row[0], date: row[1], mileage: row[2], motohours: row[3],
            parts_cost: row[4], work_cost: row[5], is_diy: row[6], notes: row[7], photo_url: row[8], timestamp: row[9]
        }));
