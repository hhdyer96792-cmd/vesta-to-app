...
function calculatePlan(op) {
    ...
    if (recMileage > settings.currentMileage) {
    ...
    if (recMotohours && recMotohours > settings.currentMotohours) {
    ...
    return {
        recDate: recDate.toISOString().split('T')[0], recMileage, recMotohours: recMotohours || '',
        planDate: planDate.toISOString().split('T')[0], planMileage: recMileage, daysLeft
    };
}
...
function updateNextServiceWidget() {
    ...
    const sorted = [...operations].sort((a,b) => calculatePlan(a).daysLeft - calculatePlan(b).daysLeft);
    const next = sorted[0]; const plan = calculatePlan(next);
...
async function saveOperation(data) {
    ...
    if (data.id) await writeSheet(`Журнал ТО!A${data.rowIndex}:H${data.rowIndex}`, [rowData]);
    else await appendSheet('Журнал ТО!A:H', [rowData]);
    await loadSheet();
}
// ==================== 12. ОФЛАЙН-ОЧЕРЕДЬ ====================
...
function renderStats() {
    // Здесь будут графики Chart.js (можно наполнить позже)
}
// ==================== 17. ОБРАБОТЧИКИ ТАБЛИЦЫ ТО ====================
