...
// ========== Глобальный наблюдатель за иконками ==========
...
function initGoogleApi() {
 ...
 // Локальный режим без авторизации
...
async function loadSheet() {
 ...
 if (!accessToken) {
 // Нет токена — работаем только из кеша
 const cached = localStorage.getItem(CACHE_KEY);
 if (cached) {
 const d = JSON.parse(cached);
...
// Гарантированная инициализация иконок после полной загрузки
