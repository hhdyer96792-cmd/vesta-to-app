...
function initEventListeners() {
  ...
  document.querySelectorAll('.nav-item').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (targetId === 'stats') { renderStats(); renderFuelAnalytics(); }
        if (targetId === 'to') renderTop5Widget();
        if (targetId === 'fuel') renderFuelTable();
        if (targetId === 'tires') renderTiresTable();
        if (targetId === 'parts') renderPartsTable();
    });
});
