...
function calculatePlan(op) {
    ...
    let avgSpeed = 30; // значение по умолчанию (смешанный режим)
...
for (const part of partsForOp) {
                ...
                // Автоматическое добавление зависимых операций (масляный фильтр, ролики ГРМ и т.д.)
...
function openTireModal(record=null) {
    ...
    form.onsubmit = (e) => { 
        ...
        // Исправлено: убрана ошибочная строка с baseMileage
        if (!isNew) {
...
function groupFuelByMonth() {
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (!isFinite(consumption)) continue; // пропустить некорректное значение
