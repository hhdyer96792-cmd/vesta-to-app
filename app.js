...
function renderStats() {
    ...
    // Рисуем график масла только если вкладка статистики активна
...
async function loadHistory() {
    if (!spreadsheetId) return;
    try {
        const raw = await readSheet('История!A2:J'); 
        const validRows=[], hData=[];
        ...
        // После загрузки – отрисовываем с учётом фильтров
        renderHistoryWithFilters();
        // Обновляем список операций в фильтре
...
function renderHistoryWithFilters() {
    ...
    filtered.sort((a,b) => (b.date || '').localeCompare(a.date || '')).forEach(record => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <button class="icon-btn edit-history-btn" data-row="${record.rowIndex}" data-opid="${record.operation_id}" data-date="${record.date}" data-mileage="${record.mileage}" data-motohours="${record.motohours}" data-parts="${record.parts_cost}" data-work="${record.work_cost}" data-diy="${record.is_diy}" data-notes="${record.notes}" data-photo="${record.photo_url}">✏️</button>
                <button class="icon-btn delete-history-btn" data-row="${record.rowIndex}">🗑️</button>
            </td>
        `;
        tbody.appendChild(tr);
    });
    // Перепривязываем события редактирования/удаления
...
function openHistoryEdit(e) {
    ...
    form.onsubmit = (ev) => {
        ...
        writeSheet(`История!A${row}:J${row}`, [newValues.concat(new Date().toISOString())])
            .then(() => { 
                loadHistory();   // перезагружает данные и перерисовывает с учётом фильтров
                loadSheet(); 
            })
...
async function deleteHistoryEntry(e) {
    ...
    loadHistory();   // обновляем таблицу с учётом фильтров
}
...
function getFilteredHistory() {
    ...
    // Фильтр по стоимости (запчасти + работа)
    filtered = filtered.filter(r => {
...
function initEventListeners() {
    ...
    // ========== НОВЫЕ ОБРАБОТЧИКИ ДЛЯ ФИЛЬТРОВ ИСТОРИИ ==========
...
// ==================== 21. ОБНОВЛЕНИЕ ПРОБЕГА И ВЛАДЕНИЕ ====================
