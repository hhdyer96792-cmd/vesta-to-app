...
async function loadSheet() {
    ...
    try {
        ...
        renderAll();
        // renderFuelAnalytics() вызывается только при переключении на вкладку статистики
    } catch (e) {
        setSyncStatus('error');
...
initGoogleApi();
initEventListeners();
if ('serviceWorker' in navigator) navigator.serviceWorker.register('service-worker.js');const ownershipUnit = document.getElementById('ownership-unit');
const toggleOwnershipUnitBtn = document.getElementById('toggle-ownership-unit');
...
async function loadSheet() {
    ...
    try {
        ...
        // Сначала показываем панель, затем отрисовываем всё
        dataPanel.style.display = 'block';
        setSyncStatus('synced');
        syncPendingActions();
        driveFolderId = await getOrCreatePhotoFolder();
