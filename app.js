...
function initGoogleApi() {
    ...
    const savedToken = localStorage.getItem('vesta_token');
    const savedSpreadsheetId = localStorage.getItem('vesta_spreadsheet_id');
    
    if (savedToken && savedSpreadsheetId) {
    ...
    authStatus.innerHTML = '⚠️ Работа без авторизации (данные только в браузере). <a href="#" id="enable-sync-link">Включить синхронизацию с Google</a>';
    ...
    if (cached) {
        ...
        tireLog = [];
        workCosts = [];
        settings = { currentMileage: 0, currentMotohours: 0, avgDailyMileage: 45, avgDailyMotohours: 1.8 };
        dataPanel.style.display = 'block';
        renderAll();
        setSyncStatus('local');
    }
    
    const enableLink = document.getElementById('enable-sync-link');
    if (enableLink) {
...
async function safeWriteSheet(range, values) {
    if (accessToken) {
        ...
        console.log('Локальный режим: запись в', range, values);
...
async function safeAppendSheet(range, values) {
    if (accessToken) {
        ...
        console.log('Локальный режим: добавление в', range, values);
...
function renderTOTable() {
    ...
    categories.forEach(cat => {
        ...
        headerRow.innerHTML = `<td colspan="7">${cat}</td>`;
    ...
    attachTOListeners();
    updateCalendarButtonsStatus().finally(() => {
        initIcons();
    });
}
...
async function addToCalendar(opName, planDate, planMileage) {
    ...
    try {
        ...
        const b=document.querySelector(`.calendar-btn[data-op-name="${opName}"][data-plan-date="${planDate}"]`);
        if(b){ 
...
async function syncPendingActions() {
    ...
    for (const a of actions) {
        try {
            ...
            } else if (a.type === 'write' || a.type === 'append') {
                if (a.type === 'write') await writeSheet(a.range, a.values);
                else await appendSheet(a.range, a.values);
                pendingActions = pendingActions.filter(act => act !== a);
            }
        } catch (e) {
...
function attachPartsListeners() {
    ...
    document.querySelectorAll('.delete-part-btn').forEach(b=>b.addEventListener('click',async e=>{ if(confirm('Удалить?')){ await safeWriteSheet(`PartsCatalog!A${b.dataset.id}:I${b.dataset.id}`,[['','','','','','','','','']]); await loadSheet(); } }));
...
function attachFuelListeners() {
    document.querySelectorAll('.delete-fuel-btn').forEach(b=>b.addEventListener('click',async e=>{ if(!confirm('Удалить?')) return; const i=b.dataset.index; await safeWriteSheet(`FuelLog!A${+i+2}:G${+i+2}`,[['','','','','','','']]); await loadSheet(); }));
...
function attachTireListeners() {
    ...
    document.querySelectorAll('.delete-tire-btn').forEach(b=>b.addEventListener('click',async e=>{ if(!confirm('Удалить?')) return; const i=b.dataset.index; await safeWriteSheet(`Tires!A${+i+2}:J${+i+2}`,[['','','','','','','','','','']]); await loadSheet(); }));
}
...
function initEventListeners() {
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (tab === 'to') { renderTop5Widget(); renderMaintenancePlan(); }
    ...
    const predictBtn = document.getElementById('calc-prediction-btn');
    if (predictBtn) {
        predictBtn.addEventListener('click', renderMileagePrediction);
    }

    const pdfBtn = document.getElementById('generate-pdf-btn');
    if (pdfBtn) {
        pdfBtn.addEventListener('click', generateServiceReport);
    }
    const generatePlanBtn = document.getElementById('generate-plan-btn');
    if (generatePlanBtn) {
        generatePlanBtn.addEventListener('click', () => renderMaintenancePlan());
    }
    const planToCalendarBtn = document.getElementById('plan-to-calendar-btn');
    if (planToCalendarBtn) {
        planToCalendarBtn.addEventListener('click', addPlanToCalendar);
    }
    ...
    if (forceSyncBtn) {
        forceSyncBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                showToast('Локальные данные выгружены в Google Таблицу', 'success');
                setSyncStatus('synced');
            } catch(e) {
                console.error(e);
                showToast('Ошибка синхронизации, проверьте авторизацию', 'error');
                setSyncStatus('error');
            }
        });
    }
...
async function updateMileageAndAverages() {
    ...
    await safeAppendSheet('MileageLog!A:C', [[today, newM, newH]]);
    ...
    await safeWriteSheet('Журнал ТО!Q1:Q4', [[settings.currentMileage],[settings.currentMotohours],[settings.avgDailyMileage],[settings.avgDailyMotohours]]);
...
async function generateServiceReport() {
    ...
    const reportHtml = `
        ...
        <body>
            ...
            <table>
                <thead>
                    <tr><th>Категория</th><th>Операция</th><th>Интервал км</th><th>Интервал мес</th><th>Последнее ТО</th><th>Последний пробег</th></tr>
                </thead>
                <tbody>
                    ${operations.map(op => `
                        <tr>
            ...
            <table>
                <thead>
                    <tr><th>Дата</th><th>Операция</th><th>Пробег</th><th>Запчасти (₽)</th><th>Работа (₽)</th><th>DIY</th><th>Примечание</th></tr>
                </thead>
                <tbody>
                    ${serviceRecords.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(rec => {
                        ...
                        return `
                            <tr>
                                ...
                                <td>${rec.mileage || ''}</td>
                                <td>${rec.parts_cost || '0'}</td>
                                <td>${rec.work_cost || '0'}</td>
                                <td>${rec.is_diy === true ? 'Да' : 'Нет'}</td>
                                <td>${rec.notes || ''}</td>
                            </tr>
                        `;
                    }).join('')}
...
function renderMaintenancePlan() {
    ...
    html += '</tbody></table>';
    container.innerHTML = html;
    document.querySelectorAll('.execute-plan-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const opId = parseInt(btn.dataset.opId);
            const opName = btn.dataset.opName;
            openServiceModal(opId, opName);
        });
    });
    const planToCalendarBtn = document.getElementById('plan-to-calendar-btn');
...
// ==================== НОВАЯ НАВИГАЦИЯ ПОЛИРОВКА (ЭТАП 4) ====================

// Функция для синхронизации активного состояния вкладок
function updateActiveTabUI(tabId) {
...
// Улучшенная версия switchToTab с дополнительной отрисовкой иконок
function switchToTab(tabId) {
    ...
    // Вызвать соответствующие функции загрузки данных
    if (tabId === 'history' && typeof loadHistory === 'function') loadHistory();
    if (tabId === 'stats') { 
        if (typeof renderStats === 'function') renderStats(); 
        if (typeof renderFuelAnalytics === 'function') renderFuelAnalytics(); 
    }
    if (tabId === 'to') { 
        if (typeof renderTop5Widget === 'function') renderTop5Widget(); 
        if (typeof renderMaintenancePlan === 'function') renderMaintenancePlan(); 
    }
    if (tabId === 'fuel' && typeof renderFuelTable === 'function') renderFuelTable();
    if (tabId === 'tires' && typeof renderTiresTable === 'function') renderTiresTable();
    if (tabId === 'parts' && typeof renderPartsTable === 'function') renderPartsTable();
    
    // Принудительная перерисовка иконок (на всякий случай)
    setTimeout(() => {
        if (typeof initIcons === 'function') initIcons();
    }, 100);
}

// Инициализация мобильной навигации (улучшенная)
function initNewNav() {
    ...
    // Обработчики для основных кнопок bottom-nav
    ...
    // Обработчики для пунктов drawer
    ...
    // Вспомогательные кнопки в drawer
    const drawerSelectCar = document.getElementById('drawer-select-car');
    const drawerTheme = document.getElementById('drawer-theme-toggle');
    const drawerSync = document.getElementById('drawer-sync');
    
    if (drawerSelectCar) drawerSelectCar.addEventListener('click', () => {
        if (typeof openCarSelectModal === 'function') openCarSelectModal();
        closeDrawer();
    });
    if (drawerTheme) drawerTheme.addEventListener('click', () => {
        const isDark = document.body.classList.contains('dark');
        if (typeof applyTheme === 'function') applyTheme(isDark ? 'light' : 'dark');
        closeDrawer();
    });
    if (drawerSync) drawerSync.addEventListener('click', () => {
        const forceSyncBtn = document.getElementById('force-sync-btn');
        if (forceSyncBtn) forceSyncBtn.click();
        closeDrawer();
    });
}

// Инициализация десктопного сайдбара (улучшенная)
function initDesktopSidebar() {
    const sidebarItems = document.querySelectorAll('.sidebar-item');
    const sidebarThemeBtn = document.getElementById('sidebar-theme');
    const sidebarSelectCarBtn = document.getElementById('sidebar-select-car');
    const sidebarSyncBtn = document.getElementById('sidebar-sync');
    ...
    if (sidebarSelectCarBtn) {
        sidebarSelectCarBtn.addEventListener('click', () => {
            if (typeof openCarSelectModal === 'function') openCarSelectModal();
        });
    }
    if (sidebarSyncBtn) {
    ...
    document.querySelectorAll('.tab-content').forEach(tab => {
        if (tab.classList.contains('active')) {
            ...
            if (id && id.startsWith('tab-')) {
                activeTabId = id.replace('tab-', '');
            }
        }
    });
    updateActiveTabUI(activeTabId);
}
...
// Переопределяем старые обработчики табов
...
// ==================== РЕГУЛИРОВКА ШИРИНЫ СТОЛБЦОВ (только для десктопа) ====================
const COLUMN_WIDTHS_KEY = 'vesta_column_widths';

// Проверка: десктопная версия (ширина экрана ≥ 768px)
function isDesktop() {
    return window.innerWidth >= 768;
}

// Инициализация ручек изменения ширины (только на десктопе)
function initColumnResizing() {
    if (!isDesktop()) return; // на мобилках не создаём ручки
    
    const tables = ['maintenance-table', 'history-table', 'fuel-table', 'tires-table', 'parts-table'];
    tables.forEach(tableId => {
        ...
        headers.forEach((th, idx) => {
            ...
            handle.addEventListener('mousedown', (e) => {
                startResize(e, th, idx, tableId);
                e.preventDefault();
                e.stopPropagation();
            });
            th.appendChild(handle);
            
            // Восстанавливаем сохранённую ширину (если есть)
            const savedWidth = getSavedColumnWidth(tableId, idx);
            if (savedWidth) {
...
function startResize(e, th, colIndex, tableId) {
    if (!isDesktop()) return;
    const startX = e.clientX;
    resizeData = { th, colIndex, tableId, startX, startWidth: th.offsetWidth };
...
function onResize(e) {
    ...
    let newWidth = resizeData.startWidth + delta;
    ...
    const table = document.getElementById(resizeData.tableId);
    if (table) {
        const rows = table.querySelectorAll('tr');
        rows.forEach(row => {
            const cell = row.cells[resizeData.colIndex];
            if (cell && row !== resizeData.th.parentElement) {
...
function stopResize() {
    if (!resizeData) return;
    saveColumnWidth(resizeData.tableId, resizeData.colIndex, resizeData.th.offsetWidth);
...
function getSavedColumnWidth(tableId, colIdx) {
    const stored = localStorage.getItem(COLUMN_WIDTHS_KEY);
    if (stored) {
        try {
            const map = JSON.parse(stored);
            return map[tableId]?.[colIdx] || null;
        } catch(e) { return null; }
    }
    return null;
}

function saveColumnWidth(tableId, colIdx, width) {
    const stored = localStorage.getItem(COLUMN_WIDTHS_KEY);
    let map = stored ? JSON.parse(stored) : {};
    if (!map[tableId]) map[tableId] = {};
    map[tableId][colIdx] = width;
    localStorage.setItem(COLUMN_WIDTHS_KEY, JSON.stringify(map));
}

let resizeObserver = null;
function initAllResizing() {
    setTimeout(() => { initColumnResizing(); }, 500);
    if (resizeObserver) resizeObserver.disconnect();
    resizeObserver = new MutationObserver(() => {
        if (isDesktop()) initColumnResizing();
    });
    resizeObserver.observe(document.body, { childList: true, subtree: true });
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initAllResizing);
} else {
    initAllResizing();
}
// ==================== ДОПОЛНИТЕЛЬНЫЙ ОБРАБОТЧИК ДЛЯ ПУНКТА "ПОДЕЛИТЬСЯ ТАБЛИЦЕЙ" В DRAWER ====================
// Добавить эту функцию в существующую инициализацию initNewNav() или добавить отдельно

function initDrawerShareButton() {
    const drawerShareBtn = document.getElementById('drawer-share-table');
    if (drawerShareBtn) {
        drawerShareBtn.addEventListener('click', () => {
            const spreadsheetId = localStorage.getItem('vesta_spreadsheet_id');
            if (spreadsheetId) {
                window.open(`https://docs.google.com/spreadsheets/d/${spreadsheetId}/edit`, '_blank');
            } else {
                showToast('Таблица не выбрана', 'warning');
            }
            // Закрыть drawer после клика
            const drawer = document.getElementById('drawer-menu');
...
// Вызвать эту функцию после загрузки DOM
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initDrawerShareButton);
} else {
