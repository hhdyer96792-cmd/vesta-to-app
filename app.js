...
// ==================== 9-А. ТАБЛИЦА ЗАПЧАСТЕЙ (с учётом склада) ====================
function renderPartsTable() {
    ...
    parts.forEach(p => {
        ...
        tr.innerHTML = `
            ...
            <td>${p.supplier}</td>
            <td>${p.link ? `<a href="${p.link}" target="_blank">🔗</a>` : ''}</td>
            <td>${p.comment}</td>
            <td style="text-align:center;">${p.inStock || 0}</td>
            <td>${p.location || ''}</td>
            <td>
                <button class="icon-btn edit-part-btn" data-id="${p.id}">✏️</button>
                <button class="icon-btn delete-part-btn" data-id="${p.id}">🗑️</button>
                <button class="icon-btn search-part-btn" data-oem="${p.oem}">🔍</button>
            </td>
        `;
        tbody.appendChild(tr);
    });
...
// ==================== 10-А. ФОРМА ЗАПЧАСТИ (с учётом склада) ====================
