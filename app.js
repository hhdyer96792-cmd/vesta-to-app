...
function updateNextServiceWidget() {
    ...
    document.getElementById('progress-bar').style.width = (isNaN(percent) ? 0 : percent) + '%';
...
function openHistoryEdit(e) {
    ...
    const modal = createModal('✏️ Редактировать запись истории', `
        <form id="history-edit-form">
            ...
            <label>Дата (ДД.ММ.ГГГГ)</label>
            <input type="text" name="date" value="${date}" pattern="\\d{2}\\.\\d{2}\\.\\d{4}" required>
...
function attachPartsListeners() {
    document.querySelectorAll('.edit-part-btn').forEach(b => {
    ...
    document.querySelectorAll('.delete-part-btn').forEach(b => {
        b.addEventListener('click', async e => {
            const id = b.dataset.id;
            if (!confirm('Удалить запчасть?')) return;
            await writeSheet(`PartsCatalog!A${id}:G${id}`, [['','','','','','','']]);
            await loadSheet();
        });
    });
    document.querySelectorAll('.search-part-btn').forEach(b => {
        b.addEventListener('click', e => {
            ...
            if (oem) window.open(`https://exist.ru/price/?pcode=${oem}`, '_blank');
        });
    });
}
...
function initEventListeners() {
    ...
    // Кнопка "Сменить резину"
    document.getElementById('add-tire-btn').onclick = () => {
        const type = prompt('Введите тип резины (лето/зима):');
        if (!type) return;
        const date = new Date().toISOString().split('T')[0];
        const mileage = settings.currentMileage;
        appendSheet('Tires!A:D', [[date, type, mileage, '']]);
        loadSheet();
    };
}
