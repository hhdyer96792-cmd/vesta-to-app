...
function formatTireInput(inputElement) {
    let val = inputElement.value.replace(/\D/g, ''); // только цифры
    if (val.length === 0) return '';
    let formatted = '';
    if (val.length <= 3) {
        formatted = val; // только ширина
...
function renderTireCalculator() {
    ...
    newBtn.addEventListener('click', () => {
        ...
        // Приводим введённые строки к каноническому формату для парсера
