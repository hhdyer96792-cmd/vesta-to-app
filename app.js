...
function addPendingAction(action) {
    ...
    setSyncStatus('error');
    showToast('Действие добавлено в очередь синхронизации', 'warning');
}
...
async function syncPendingActions() {
    ...
    for (let i = 0; i < pendingActions.length; i++) {
        ...
        try {
            ...
            a.retryCount++;
            if (a.retryCount < 5) {
                const delay = Math.min(30000, 1000 * Math.pow(2, a.retryCount));
                setTimeout(() => syncPendingActions(), delay);
            } else {
                showToast('Не удалось синхронизировать действие, оно будет пропущено', 'error');
                pendingActions.splice(i,1);
                i--;
                localStorage.setItem(PENDING_KEY, JSON.stringify(pendingActions));
            }
