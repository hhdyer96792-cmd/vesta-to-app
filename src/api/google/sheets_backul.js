...
App.loadSheet = function() {
    ...
    return Promise.all([
        App.api.readSheet(App.config.SHEET_MAINTENANCE + '!A2:O'),   // ← изменено на O
        ...
        // Операции – столбец O = updated_at (индекс 14)
...
App.api.syncPendingActions = function() {
    ...
    App.setSyncStatus('syncing');
    var promises = pending.map(function(action) {
        if (action.type === 'write') return App.api.writeSheet(action.range, action.values);
        if (action.type === 'append') return App.api.appendSheet(action.range, action.values);
    });
...
App.api.syncAllToSheet = function() {
    ...
    return Promise.all([
        App.api.safeWriteSheet(App.config.SHEET_MAINTENANCE + '!A2:O', opsRows),
        App.api.safeWriteSheet(App.config.RANGE_SETTINGS, settingsRow)
    ]);
};